...
# make it large to see if it is compiled as a loop
seq_size = 2
batch_size = 1
input_size = 10
hidden_size = 4
...
def make_rnn():
    w = weight_scale * np.ones((1, hidden_size, input_size)).astype(np.float32).flatten()
    r = weight_scale * np.ones((1, hidden_size, hidden_size)).astype(np.float32).flatten()
    ...
    graph_def = helper.make_graph(
        ...
        [
            helper.make_tensor('w', TensorProto.FLOAT,
                                [1, hidden_size, input_size], w),
            helper.make_tensor('r', TensorProto.FLOAT, [1, hidden_size, hidden_size], r)
        ]
    )
